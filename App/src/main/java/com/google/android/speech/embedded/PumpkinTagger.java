...
import com.google.speech.grammar.pumpkin.PumpkinTaggerResultsProto.HypothesisResult;
import com.google.speech.grammar.pumpkin.PumpkinTaggerResultsProto.PumpkinTaggerResults;
        ...
        this.mWorkerThread.execute(new Runnable()
        {
          public void run()
          {
            ...
            PumpkinTagger.this.mUiThread.execute(new Runnable()
            {
              public void run()
              {
                SimpleCallback localSimpleCallback = PumpkinTagger.1.this.val$callback;
        ...
        for (final TaggerResult localTaggerResult = new TaggerResult(localPumpkinTaggerResults.getHypothesis(0), localEmbeddedParserDetails);; localTaggerResult = null)
        {
          PumpkinTagger.this.mUiThread.execute(new Runnable()
          {
            public void run()
            {
              ...
              {
                PumpkinTagger.2.this.val$disambiguationFailureCallback.onResult(null);
                return;
              }
