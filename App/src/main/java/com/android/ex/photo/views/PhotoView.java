  ...
  implements GestureDetector.OnDoubleTapListener, GestureDetector.OnGestureListener, ScaleGestureDetector.OnScaleGestureListener
{
  ...
  private void generateMatrix()
  {
    ...
    for (;;)
    {
      ...
      for (;;)
      {
        RectF localRectF = new RectF(k / 2 - i * this.mMaxInitialScaleFactor / 2.0F, m / 2 - j * this.mMaxInitialScaleFactor / 2.0F, k / 2 + i * this.mMaxInitialScaleFactor / 2.0F, m / 2 + j * this.mMaxInitialScaleFactor / 2.0F);
  ...
  {
    if ((paramDrawable != null) && (paramDrawable != this.mDrawable))
    {
  ...
  {
    boolean bool1 = this.mDrawable instanceof BitmapDrawable;
    if (!bool1) {
      bool2 = true;
    }
    while ((this.mDrawable != null) && (bool1)) {
      if (paramBitmap == ((BitmapDrawable)this.mDrawable).getBitmap())
      {
        return;
        bool2 = false;
      }
    ...
    implements Runnable
  {
    ...
    public void run()
    {
      ...
      for (;;)
      {
        ...
        break label34;
        label190:
        this.mVelocityX = (f2 + this.mVelocityX);
        if (this.mVelocityX <= 0.0F) {
          break label100;
        }
        this.mVelocityX = 0.0F;
