  ...
  implements Transport
{
    ...
    throws IOException
  {
    ...
    if (bool)
    {
      int i = this.httpEngine.policy.getChunkLength();
      if (i == -1) {
        i = 1024;
      }
      writeRequestHeaders();
      return new ChunkedOutputStream(this.requestOut, i, null);
    }
    long l1 = this.httpEngine.policy.getFixedContentLength();
    if (l1 != -1L)
    {
      this.httpEngine.requestHeaders.setContentLength(l1);
      writeRequestHeaders();
      return new FixedLengthOutputStream(this.requestOut, l1, null);
    }
    long l2 = this.httpEngine.requestHeaders.getContentLength();
    if (l2 > 2147483647L) {
